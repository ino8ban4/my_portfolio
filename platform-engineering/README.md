...
| モニタリング | Prometheus + Grafana | メトリクス収集・可視化（実装予定） |
| セキュリティ | NetworkPolicy / Trivy | ネットワーク制御・脆弱性スキャン |
...
### NetworkPolicy
不要なPod間通信を遮断するためNetworkPolicyをHelmチャートに組み込んでいます。

- Ingress: port 3000のみ許可
- Egress: DNS（port 53）のみ許可、それ以外の外向き通信は遮断

### 脆弱性スキャン（Trivy）
GitHub ActionsのCIパイプラインにTrivyを組み込み、ビルドごとにコンテナイメージの脆弱性を自動検出しています。

- 対象: ghcr.ioにpushしたイメージ
- 検出レベル: HIGH / CRITICAL
- 現在は検知のみ（exit-code: 0）、本番運用では0以上でCIブロック予定
